...
define(function(require, exports, module) {
	...
	var appendImg = function(opt, img) {
		var html = $('<div class="col-xs-4 col-sm-3 col-md-2 img_item"><div class="item" ><img src="'+img+'" /><span class="display: block; height: 100%;" ></span><div class="loading" ><img src="'+opt.img_loading_url+'" /></div><div class="error" ><span class="glyphicon glyphicon-warning-sign" ></span></div><div class="toolbar" ><span class="glyphicon glyphicon-remove" ></span></div></div></div>');
		$(opt.target).find(".imglist").append(html);
		return html;
	};
		...
		$(opt.target).on("click", ".btn_add", function() {
			fileserver.upload_base64({
				...
				onSelect : function(base64, file, reader) {
					...
					$(opt.target).find(".btn_add").parents(".img_item").appendTo($(opt.target).find(".imglist"));
					//
					recheckAddBtnState(opt);
					return true;
				},
				onUpload : function(url, basepath, file) {
					...
					$(opt.target).find(".img_item[fileid="+file.fileId+"] img").attr("src", basepath+url);
					$(opt.target).find(".img_item[fileid="+file.fileId+"] .loading").hide();
					//
					reloadImgArray(opt);
				},
				onError : function(state, file) {
					$(opt.target).find(".img_item[fileid="+file.fileId+"] .loading").hide();
					$(opt.target).find(".img_item[fileid="+file.fileId+"] .error").show();
				}
			});

...
define(function(require, exports, module) {
	...
	*/
	...
	var emoji_code_to_html = function(e, options) {
		var opt = $.extend(defaults, options);
		if (!e) e = $("body");
		var html = $(e).html();
		var re = new RegExp("\\"+opt.emoji_tag_start+"([^\\"+opt.emoji_tag_end+"]*)\\]", "gi");
		html = html.replace(re, '<span class="emoji" emoji="$1" emoji_state="code">'+opt.emoji_tag_start+'$1'+opt.emoji_tag_end+'</span>');
		$(e).html(html);
	};
	...
	*/
	...
	var dialog = function(options) {
		...
		$.when( 
			...
			emoji_data.opt = opt;
			var htmlContent = template.render(tmpl_content)(emoji_data);
			var dialog = $('<div></div>').append(htmlContent);  
			// 生成页码
	...
	module.exports = {
		dialog : dialog,
		emoji_code_to_html : emoji_code_to_html,

...
org.activemq.Amq = function() {
	...
	var adapter = require("apache/amq/amq_jquery_adapter.js");
  ...
  var ajaxHandler;
	...
	var pollErrorHandler = function(xhr, status, ex) {
		...
		if (status === 'error' && xhr.status === 0) {
			...
			timeoutHandler = setTimeout(function() { sendPoll(); }, pollErrorDelay);
			return;
		}
	...
	var pollHandler = function(data) {
		try {
			...
			timeoutHandler = setTimeout(sendPoll, pollDelay);
		}
	};
	...
	var sendPoll = function() {
		...
		ajaxHandler = adapter.ajax(uri, options);
	};
	...
	var sendJmsMessage = function(destination, message, type, headers) {
		...
		if (batchInProgress) {
			...
			ajaxHandler = adapter.ajax(uri, { method: 'post',
	...
	module.exports = org.activemq.Amq;

	...
	var init = function(options) {
		...
		loadTmpl(opt.path_tmpl, opt, function(html) {
			...
			WebUpoaderLoader.init(function(WebUploader) {
				...
				opt.uploader.on( 'all', function(type, file) {
					var stats;
					switch( type ) {
						case 'uploadFinished':
						...
						case 'beforeFileQueued' : 
							...
							if (
									(function() {
										var s=[];
										for(var i=0; i<opt.alloc_file_ext.length; i++) {
											s.push(opt.alloc_file_ext[i].toLowerCase());
										}
										return s;
									})()
									.indexOf(file.ext.toLowerCase()) == -1) {
								alert("文件格式错误");
								return false;
							}
							break;
		...
		$btns.on( 'click', 'span', function() {
			...
			switch ( index ) {
				case 0:
					...
					var server_url = $(this).parents(".state-complete").attr("server_url")
					if (server_url && server_url != '') {
						if (opt.imgarray.indexOf(server_url) > -1) {
							opt.imgarray.remove(opt.imgarray.indexOf(server_url));
						}
					}
					return;
		...
		switch ( opt.state ) {
			...
			case 'finish':
				...
				if ( stats.successNum ) {
					...
					for (var i=0; i<opt.imgarray.length; i++) {
						var img = opt.imgarray[i];
						$.each($(".filelist li"), function(k, v) {
							$(v).attr("server_url", img);
						});
					}
